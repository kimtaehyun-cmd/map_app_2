...
function startLenderMap(lat, lng) {
  ...
  CurrentData.forEach((item) => {
    ...
    if (bounds.hasLatLng(latlng)) {
      //현재 위치를 기준으로 화면사이에 있는지 확인. 화면 내부의 마커만 생성
      var marker = new naver.maps.Marker({
        position: latlng,
        map: map,
      ...
      let infoWindow = new naver.maps.InfoWindow({
        content: `
        <h4 stle="padding:0.25rem; 0.5rem; font-size:12px;
      ...
      naver.maps.Event.addListener(marker, 'click', function () {
        ...
        getInfoOnMaker(markerInfoData);
      });
    }
  });

  function getInfoOnMaker(markerInfoData) {
